...
def load_settings():
    if not os.path.exists(SETTINGS_FILE):
        return {"n_days": 7, "threshold_mm": 20, "data_file": "rain_data.csv"}
...
class RainApp(tk.Tk):
    ...
    def _save_data(self):
        fieldnames = ["Date", "Rain_mm", "BOM_mm", "Notes"]
    ...
    def _build_ui(self):
        ...
        columns = ("Date", "Rain_mm", "BOM_mm", "Effective_mm", "Notes")
        ...
        tk.Label(dash_frame, text="N days:").grid(row=0, column=0, sticky="e")
        self.entry_n_days = tk.Entry(dash_frame, width=5)
        self.entry_n_days.grid(row=0, column=1, padx=5)
        ...
        self.entry_n_days.insert(0, str(self.settings["n_days"]))
        self.entry_threshold.insert(0, str(self.settings["threshold_mm"]))

        tk.Label(dash_frame, text="Rain last N days:").grid(row=1, column=0, sticky="e")
        self.lbl_rain_last_n = tk.Label(dash_frame, text="0.0")
        self.lbl_rain_last_n.grid(row=1, column=1, sticky="w")
        ...
        tk.Label(dash_frame, text="Last rainfall date:").grid(row=2, column=0, sticky="e")
        self.lbl_last_rain_date = tk.Label(dash_frame, text="-")
        self.lbl_last_rain_date.grid(row=2, column=1, sticky="w")

        tk.Label(dash_frame, text="Days since last rain:").grid(row=2, column=2, sticky="e")
        self.lbl_days_since = tk.Label(dash_frame, text="-")
        self.lbl_days_since.grid(row=2, column=3, sticky="w")

        tk.Label(dash_frame, text="Missing days:").grid(row=3, column=0, sticky="e")
        self.lbl_missing = tk.Label(dash_frame, text="-")
        self.lbl_missing.grid(row=3, column=1, sticky="w")

        btn_show_missing = tk.Button(dash_frame, text="Show Missing Dates", command=self._show_missing_dates)
        btn_show_missing.grid(row=3, column=2, columnspan=2, pady=3)

        # Recalculate dashboard when N or threshold changes
        self.entry_n_days.bind("<FocusOut>", lambda e: self._update_dashboard())
    ...
    def _refresh_table(self):
        ...
        for rec in self.records:
            ...
            self.tree.insert(
                "", tk.END,
                values=(rec["Date"], rec["Rain_mm"], rec["BOM_mm"], eff_str, rec["Notes"])
            )

    def _update_dashboard(self):
        # Rain last N days
        try:
            n_days = int(self.entry_n_days.get())
        except ValueError:
            n_days = 7
            self.entry_n_days.delete(0, tk.END)
        ...
        # ---------------------------------------------------------
        # SAVE SETTINGS BACK TO settings.json (correct placement)
        # ---------------------------------------------------------
        self.settings["n_days"] = n_days
        self.settings["threshold_mm"] = threshold
        save_settings(self.settings)
        # ---------------------------------------------------------

        # --- Now update the dashboard using the validated settings ---
        today = date.today()
        ...
        self.lbl_rain_last_n.config(text=f"{total:.1f}")
